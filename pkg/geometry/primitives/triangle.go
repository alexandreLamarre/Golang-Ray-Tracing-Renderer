...
func (t *Triangle) GetBounds() (*algebra.Vector, *algebra.Vector) {
	...
	for _, p := range points {
		xMin = math.Min(p.Get()[0], xMin)
		yMin = math.Min(p.Get()[1], yMin)
		zMin = math.Min(p.Get()[2], zMin)
		xMax = math.Max(p.Get()[0], xMax)
		yMax = math.Max(p.Get()[1], yMax)
		zMax = math.Max(p.Get()[2], zMax)
	}
