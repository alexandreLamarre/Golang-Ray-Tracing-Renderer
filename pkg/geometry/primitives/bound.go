...
func GetBoundsTransform(min, max *algebra.Vector, transform *algebra.Matrix) *Bounds {
	...
	var newMinX float64 = math.Inf(1)
	var newMinY float64 = math.Inf(1)
	var newMinZ float64 = math.Inf(1)
	var newMaxX float64 = math.Inf(-1)
	var newMaxY float64 = math.Inf(-1)
	var newMaxZ float64 = math.Inf(-1)
	for i := 0; i < len(vertices); i++ {

		newVertex := transform.MultiplyByVec(vertices[i])
		newX := newVertex.Get()[0]
		newY := newVertex.Get()[1]
		newZ := newVertex.Get()[2]
		newMinX = math.Min(newMinX, newX)
		newMinY = math.Min(newMinY, newY)
		newMinZ = math.Min(newMinZ, newZ)
		newMaxX = math.Max(newMaxX, newX)
		newMaxY = math.Max(newMaxY, newY)
		newMaxZ = math.Max(newMaxZ, newZ)
	}
...
func (b *Bounds) Intersect(ray *algebra.Ray) bool {
	...
	minX := b.minimum.Get()[0]
	minY := b.minimum.Get()[1]
	minZ := b.minimum.Get()[2]
	maxX := b.maximum.Get()[0]
	maxY := b.maximum.Get()[1]
	maxZ := b.maximum.Get()[2]
