...
// Implementation of Perlin Noise based on Ken Perlin's improved perlin noise - COPYRIGHT 2002 KEN PERLIN.
// The implementation by Ken Perlin can be found at : https://mrl.cs.nyu.edu/~perlin/noise/

import (
	"math"
)

//Perlin returns the noise generated by a 3d point
func Perlin(x, y, z float64) float64 {
	minVal := math.Min(x, y)
	minVal = math.Min(minVal, z)
	x0 := int(math.Floor(math.Abs(x)))
	y0 := int(math.Floor(math.Abs(y)))
	z0 := int(math.Floor(math.Abs(z)))
	x -= math.Floor(x)
	y -= math.Floor(y)
	z -= math.Floor(z)
	u := fade(x)
...
func interpolate(a, b, w float64) float64 {
	return a + (b-a)*w
}
