...
func Simplex3Noise(xr, yr, zr float64, seed int64) float64{
	...
	c := LOOKUP3d[index]
	for c != nil{
		dxr := xri + c.dxr; dyr := yri + c.dyr; dzr := zri + c.dzr
		attn := 0.5 - dxr * dxr - dyr * dyr - dzr * dzr
		if attn < 0{
			c = c.nextOnFailure
		} else {
			pxm := int(xrb) + c.xrv &PMASK; pym := (int(yrb) + c.yrv) & PMASK; pzm := (int(zrb) + c.zrv) &PMASK
...
func newLattice() *lattice{
	...
	for i := 0; i < 8; i++{
		...
		c5.nextOnFailure = c6; c5.nextOnSuccess = nil
		c6.nextOnFailure = c7; c6.nextOnSuccess = nil
...
func newGradient3D() [][3]float64{
	...
	for  i := 0; i < len(grad3); i++ {
		grad3[i][0] /= N3; grad3[i][1] /= N3; grad3[i][2] /= N3
	}
	for  i := 0; i < PSIZE; i++ {
